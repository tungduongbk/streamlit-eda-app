...
class YFinanceStockHistoryRepository(CompanyStockHistoryDAO, ABC):
    ...
    async def get_history_by_ticker_and_period(self, ticker: str,
                                               period: Period = Period.one_month) -> Optional[CompanyStockHistory]:
        ticker_info = yf.Ticker(ticker, asynchronous=True,
                                backoff_factor=0.2, retry=5, max_workers=4)
        df_history = ticker_info.history(period=period.value, interval=period.interval.value)
        ...
        return CompanyStockHistory(symbol=ticker, delta=delta, delta_percentage=delta_percentage, history=time_series)

    async def search_company_history(self, query, period: Period = Period.one_month) -> Optional[CompanyStockHistory]:
        company = await company_repository.search(query)
        if company is None:
            return
        company_history = await self.get_history_by_ticker_and_period(company.symbol, period)
    ...
    def _get_timeseries_data(df: pd.DataFrame) -> List[TimeSeries]:
        if df.shape[0] > 0:
            ...
            if len(records) > 0:
                ...
                return [TimeSeries(timestamp=record["date"], value=record["close"]) for record in records]
        return []
