...
df_item_sales_by_year = (
    df[["Item_Outlet_Sales", "Outlet_Establishment_Year"]]
    .groupby("Outlet_Establishment_Year")
    .sum()
    .reset_index()
    .sort_values(by=["Outlet_Establishment_Year"])
)

df_item_sales_by_item_type = (
    df[["Item_Outlet_Sales", "Item_Type"]]
    .groupby("Item_Type")
    .sum()
    .reset_index()
)
...
def app():
    ...
    with col2:
        ...
        fig = px.pie(
            df, values="Item_Outlet_Sales", names="Outlet_Location_Type"
        )
        st.plotly_chart(fig, use_container_width=True)
    ...
    fig = px.line(
        data_frame=df_item_sales_by_year,
        y="Item_Outlet_Sales",
...
def plot_revenue_by_item_type():
    fig = px.bar(
        data_frame=df_item_sales_by_item_type,
        y="Item_Outlet_Sales",
        x="Item_Type",
